...
export async function listTestPlans(
  ...
  try {
    ...
    const apiToken = (testPlans as any).continuationToken as string | undefined;
    const truncated = testPlans.slice(0, top);
    const hasMoreResults = truncated.length < testPlans.length || !!apiToken;
    ...
    throw new AzureDevOpsError(
      `Failed to list test plans: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
